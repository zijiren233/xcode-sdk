...
if [ -z "$XCODE_TOOLS" ]; then
  ...
  if [ -d "Contents/Developer/Platforms/$PLATFORM.platform/Developer/SDKs" ]; then
    pushd "Contents/Developer/Platforms/$PLATFORM.platform/Developer/SDKs" &>/dev/null
  else
    if [ -d "../Packages" ]; then
      ...
      if [ $? -ne 0 ]; then
        ...
        SDKDIR=$(find . -name SDKs -type d | grep $PLATFORM | head -n1)
